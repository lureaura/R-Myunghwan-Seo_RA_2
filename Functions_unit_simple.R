...
simulation2.b1<-function(r1, sam, gm0, rho0, tht0, b, lag, mu, mod.type){ #mod.type =="size" or "power"
  ...
  if(mod.type=="size"){
    ...
    for(am in 1:length(rho0)){
      ...
      sim.res<-simu.null.b1(r1, n, rho, tht, b, lag, mu, kn, ngrid)
      for(i in 1:2){
    ...
    cat("<< Rejection frequency >>\n", file="Test of size_unit result(simple).csv")
    cat(p5, file="Test of size_unit result(simple).csv",append = T,"\n")
    write.table(as.matrix(res5.a),"Test of size_unit result(simple).csv",append = T,sep = ",",col.names = F)
    cat(p10, file="Test of size_unit result(simple).csv",append = T,"\n")
    write.table(as.matrix(res10.a),"Test of size_unit result(simple).csv",append = T,sep = ",",col.names = F)
    return(list.res)
  }else{
    ...
    for(ig in 1:length(gm0)){
      ...
      for(am in 1:length(rho0)){
        rho<-rho0[am];  tht<-tht0[am]
        sim.res<-simu.alt.b1(r1, n, gm, ap, rho, tht, b, lag, mu, kn, ngrid)
        for(i in 1:2){
    ...
    cat("<< Rejection frequency >>\n", file="Test of power_unit result(simple).csv")
    cat(p5.4, file="Test of power_unit result(simple).csv",append = T,"\n")
    write.table(as.matrix(res5.4a),"Test of power_unit result(simple).csv",append = T,sep = ",",col.names = F)
    cat(p10.4, file="Test of power_unit result(simple).csv",append = T,"\n")
    write.table(as.matrix(res10.4a),"Test of power_unit result(simple).csv",append = T,sep = ",",col.names = F)
    cat(p5.8, file="Test of power_unit result(simple).csv",append = T,"\n")
    write.table(as.matrix(res5.8a),"Test of power_unit result(simple).csv",append = T,sep = ",",col.names = F)
    cat(p10.8, file="Test of power_unit result(simple).csv",append = T,"\n")
    write.table(as.matrix(res10.8a),"Test of power_unit result(simple).csv",append = T,sep = ",",col.names = F)
...
simu.null.b1<-function(r1, n, rho, tht, b, lag, mu, kn, ngrid){  
  wn<-adf1<-matrix(0,r1,2)
  lapply(1:r1, function(i){
    xs<-as.matrix(dgp_lin(n, rho, tht, mu))
    wn[i,1]  <<- rbb.bnd.b1(xs, lag, kn, ngrid, b)[[1]]
    adf1[i,1]<<- rbb.adf.b1(xs, lag ,b)[[1]]
    wn[i,2]  <<- rbb.bnd.b1(xs, lag, kn, ngrid, b)[[2]]
    adf1[i,2]<<- rbb.adf.b1(xs, lag ,b)[[2]]
  })
  
  crt.wn <- quantile(wn[,2],c(0.9,0.95))
  crt.adf<- quantile(adf1[,2],c(0.1,0.05))
  
  rej.wn10 <- sum(wn[,1]>crt.wn[1])/r1 
  rej.adf10<- sum(adf1[,1]<crt.adf[1])/r1 
  rej.wn5 <- sum(wn[,1]>crt.wn[2])/r1 
  rej.adf5<- sum(adf1[,1]<crt.adf[2])/r1 
...
simu.alt.b1<-function(r1, n, gm, ap, rho, tht, b, lag, mu, kn, ngrid){
  wn<-adf1<-matrix(0,r1,2)
  lapply(1:r1, function(i){
...
block.boot.b1<-function(data, lag, kn, ngrid, bl, mod.b, mod.boot){ #mod.b : fix or opt / mod.boot : bnd or adf
  if(mod.b=="fix" & mod.boot=="bnd"){
    b<-bl
    return(rbb.bnd.b1(data, lag, kn, ngrid, b))
  }
  
  if(mod.b=="fix" & mod.boot=="adf"){
    b<-bl
    return(rbb.adf.b1(data, lag ,b))
  }
...
# OUTPUT : supwald and RBB-supwald / ADF and RBB-ADF test statistic 
rbb.bnd.b1<-function(data, lag, kn, ngrid, b){
  ...
  return(list(wald=wald,wald.b=wald.boot))
}
rbb.adf.b1<-function(data, lag ,b){
  ...
  t.boot<-adf(y.boot, lag)[[2]]
  return(list(adf=t,adf.b=t.boot))
}
...
adf<-function(data, lag){                # model : y(t)=mu+y(t-1)+delta_y of lags
  ...
  dy_lag<-embed(diff(y),(p+1))[,(2:(p+1))]   #n-p-1 x p
